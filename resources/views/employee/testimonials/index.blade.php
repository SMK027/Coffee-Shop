            ...
            <div class="flex flex-col sm:flex-row sm:items-start gap-4">
                ...
                <div class="flex sm:flex-col gap-2 flex-shrink-0">
                    ...
                    <form action="{{ route('employee.testimonials.publish', $t) }}" method="POST">
                        @csrf @method('PATCH')
                        <button class="px-3 py-2 sm:py-1.5 bg-green-100 hover:bg-green-200 text-green-700 text-xs font-medium rounded-lg transition-colors w-full">Publier</button>
                    </form>
                    @endif
                    @if($t->status !== 'rejected')
                    <form action="{{ route('employee.testimonials.reject', $t) }}" method="POST">
                        @csrf @method('PATCH')
                        <button class="px-3 py-2 sm:py-1.5 bg-stone-100 hover:bg-stone-200 text-stone-600 text-xs font-medium rounded-lg transition-colors w-full">Rejeter</button>
                    </form>
                    @endif
                    <form action="{{ route('employee.testimonials.destroy', $t) }}" method="POST" onsubmit="return confirm('Supprimer ce témoignage ?')">
                        @csrf @method('DELETE')
                        <button class="px-3 py-2 sm:py-1.5 bg-red-50 hover:bg-red-100 text-red-600 text-xs font-medium rounded-lg transition-colors w-full">Suppr.</button>
                    </form>
                </div>
            </div>
        </div>
