<x-employee-layout title="Nouvelle note interne">
    ...
    <form action="{{ route('employee.internal-notes.store') }}" method="POST" enctype="multipart/form-data" class="max-w-3xl space-y-5" x-data="noteEditor()" @submit="syncDescription()">
        @csrf
        ...
        <div class="bg-white border border-stone-200 rounded-lg p-6 space-y-5">
            <div>
                ...
                <input id="title" name="title" value="{{ old('title') }}" maxlength="150" required class="w-full border border-stone-300 rounded-lg px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-amber-500">
            ...
            <div class="grid sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <input id="background_color" name="background_color" type="color" value="{{ old('background_color', '#FEF3C7') }}" class="w-full h-10 border border-stone-300 rounded-lg">
                </div>
                <div>
                    <label for="text_color" class="block text-sm font-medium text-stone-700 mb-1">Couleur du texte</label>
                    <input id="text_color" name="text_color" type="color" value="{{ old('text_color', '#78350F') }}" class="w-full h-10 border border-stone-300 rounded-lg">
                </div>
                <div>
                    <label for="font_family" class="block text-sm font-medium text-stone-700 mb-1">Police</label>
                    <select id="font_family" name="font_family" class="w-full border border-stone-300 rounded-lg px-3 py-2.5 text-sm">
                        @foreach(['sans-serif' => 'Sans-serif', 'serif' => 'Serif', 'monospace' => 'Monospace', 'cursive' => 'Cursive'] as $value => $label)
                            <option value="{{ $value }}" @selected(old('font_family', 'sans-serif') === $value)>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
