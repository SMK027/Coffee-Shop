<x-employee-layout :title="isset($note) ? 'Modifier la note interne' : 'Nouvelle note interne'">
    ...
    @php
        ...
        $selectedFontFamily = old('font_family', $note->font_family ?? 'Figtree, ui-sans-serif, system-ui, sans-serif');
        $selectedRecipients = old('recipient_ids', $isEditing ? $note->recipients->pluck('id')->all() : []);
        $initialAudience = old('audience', ($note->is_for_all ?? false) || ($note->display_location ?? null) === 'global_banner' ? 'all' : 'targeted');
    @endphp

    <form action="{{ $isEditing ? route('employee.internal-notes.update', $note) : route('employee.internal-notes.store') }}" method="POST" enctype="multipart/form-data" class="max-w-5xl grid lg:grid-cols-[minmax(0,1fr)_20rem] gap-5 items-start" x-data="noteEditor()" @submit="syncDescription()">
        @csrf
        ...
        <div class="bg-white border border-stone-200 rounded-lg p-6 space-y-5">
            <div>
                ...
                <input id="title" name="title" x-model="title" value="{{ old('title', $note->title ?? '') }}" maxlength="150" required class="w-full border border-stone-300 rounded-lg px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-amber-500">
            ...
            <div>
                ...
                <div class="border border-stone-300 rounded-lg overflow-hidden">
                    ...
                    <div x-ref="editor" contenteditable="true" @input="syncDescription()" class="min-h-52 p-4 text-sm text-stone-700 outline-none" aria-label="Description de la note">{!! old('description', $note->description ?? '') !!}</div>
                </div>
            ...
            <div>
                ...
                <select id="display_location" name="display_location" x-model="location" class="w-full border border-stone-300 rounded-lg px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-amber-500">
                    <option value="inbox" @selected(old('display_location', $note->display_location ?? '') === 'inbox')>Messagerie interne</option>
                    <option value="employee_banner" @selected(old('display_location', $note->display_location ?? '') === 'employee_banner')>Bannière de l'espace salarié</option>
                    <option value="global_banner" @selected(old('display_location', $note->display_location ?? '') === 'global_banner')>Bannière globale du site public</option>
                </select>
            </div>
                ...
                <div x-show="audience === 'targeted'">
                    ...
                    <select id="recipient_ids" name="recipient_ids[]" multiple class="w-full border border-stone-300 rounded-lg px-4 py-2 text-sm min-h-28">
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" @selected(in_array($user->id, $selectedRecipients))>{{ $user->name }} ({{ $user->username }})</option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="grid sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <input id="background_color" name="background_color" x-model="backgroundColor" type="color" value="{{ old('background_color', $note->background_color ?? '#FEF3C7') }}" class="w-full h-10 border border-stone-300 rounded-lg">
                </div>
                <div>
                    <label for="text_color" class="block text-sm font-medium text-stone-700 mb-1">Couleur du texte</label>
                    <input id="text_color" name="text_color" x-model="textColor" type="color" value="{{ old('text_color', $note->text_color ?? '#78350F') }}" class="w-full h-10 border border-stone-300 rounded-lg">
                </div>
                <div>
            ...
            <div>
                ...
                <input id="expires_at" name="expires_at" type="datetime-local" value="{{ old('expires_at', isset($note) && $note->expires_at ? $note->expires_at->format('Y-m-d\TH:i') : '') }}" class="w-full border border-stone-300 rounded-lg px-4 py-2.5 text-sm">
            ...
            <div>
                ...
                @if($isEditing && $note->attachments->isNotEmpty())
                    <p class="text-xs text-stone-500 mb-2">Pièces jointes conservées : {{ $note->attachments->pluck('original_name')->join(', ') }}</p>
                @endif
        ...
        <div class="lg:col-span-2 flex gap-3">
            <button type="submit" class="bg-amber-700 hover:bg-amber-600 text-white px-5 py-2.5 rounded-lg text-sm font-medium">{{ $isEditing ? 'Enregistrer les modifications' : 'Envoyer la note' }}</button>
    ...
    <script>
        function noteEditor() {
            return {
                location: @json(old('display_location', $note->display_location ?? 'inbox')),
                audience: @json($initialAudience),
                title: @json(old('title', $note->title ?? '')),
                description: @json(old('description', $note->description ?? '')),
                backgroundColor: @json(old('background_color', $note->background_color ?? '#FEF3C7')),
                textColor: @json(old('text_color', $note->text_color ?? '#78350F')),
