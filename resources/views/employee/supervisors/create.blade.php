        ...
        <div class="bg-white rounded-xl shadow-sm border border-stone-100 p-6">
            ...
            <div class="mt-5">
                <label for="holder_admin_id" class="block text-sm font-medium text-stone-700 mb-1.5">
                    Détenteur (administrateur simple) <span class="text-red-500">*</span>
                </label>
                <select name="holder_admin_id" id="holder_admin_id"
                        class="w-full border {{ $errors->has('holder_admin_id') ? 'border-red-400 bg-red-50' : 'border-stone-300' }} rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none bg-white">
                    <option value="">Sélectionner un administrateur</option>
                    @foreach($admins as $admin)
                        @php
                            $owner = $superadmins->firstWhere('id', $admin->superadmin_id);
                            $isSelected = (string) old('holder_admin_id') === (string) $admin->id;
                        @endphp
                        <option value="{{ $admin->id }}" {{ $isSelected ? 'selected' : '' }}>
                            {{ $admin->name }}{{ $owner ? ' — Responsable: ' . $owner->name : '' }}
                        </option>
                    @endforeach
                </select>
                @error('holder_admin_id')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                <p class="text-xs text-stone-400 mt-1">Le détenteur et le super-administrateur responsable pourront tous deux gérer ce superviseur.</p>
            </div>
        </div>
