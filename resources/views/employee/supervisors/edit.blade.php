        ...
        @if($isSuperAdmin)
            ...
            <div class="bg-white rounded-xl shadow-sm border border-stone-100 p-6">
                ...
                <select name="holder_admin_id" id="holder_admin_id"
                        class="w-full border {{ $errors->has('holder_admin_id') ? 'border-red-400 bg-red-50' : 'border-stone-300' }} rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none">
                    <option value="">Aucun (détenu directement par le responsable)</option>
                    @foreach($holderAdmins as $admin)
                        <option value="{{ $admin->id }}" @selected((int) old('holder_admin_id', $supervisor->holder_admin_id) === $admin->id)>{{ $admin->name }}</option>
                    @endforeach
                </select>
