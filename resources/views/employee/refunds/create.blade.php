<x-employee-layout title="Remboursement — Commande #{{ str_pad($order->id, 4, '0', STR_PAD_LEFT) }}">
    <x-slot name="headerActions">
        <a href="{{ route('employee.refunds.index') }}" class="text-stone-500 hover:text-stone-700 text-sm">← Retour aux remboursements</a>
    </x-slot>
    ...
    <div class="max-w-2xl space-y-6" x-data="refundForm()">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-stone-100 p-5">
            <div class="flex items-start justify-between gap-3">
                <div>
                    <h2 class="font-semibold text-stone-800">Commande #{{ str_pad($order->id, 4, '0', STR_PAD_LEFT) }}</h2>
                ...
                <a href="{{ route('employee.orders.show', $order) }}"
                   class="flex-shrink-0 text-xs text-stone-400 hover:text-amber-700 transition-colors border border-stone-200 hover:border-amber-300 px-2.5 py-1.5 rounded-lg">
                    Voir la commande
                </a>
            </div>
            ...
            @if($order->payments->isNotEmpty())
                <div class="mt-3 pt-3 border-t border-stone-100">
                    <p class="text-xs text-stone-500 font-medium mb-1">Paiements d'origine</p>
                    <div class="flex flex-wrap gap-2">
                        @foreach($order->payments as $payment)
                            <span class="bg-stone-100 text-stone-600 text-xs px-2.5 py-1 rounded-full">
                                {{ $payment->paymentMethod->name ?? '—' }} : {{ number_format($payment->amount, 2, ',', ' ') }} €
                            </span>
                        @endforeach
                    </div>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-stone-100 p-5">
            ...
            <form action="{{ route('employee.refunds.store', $order) }}" method="POST">
                ...
                {{-- Inputs cachés générés via x-for --}}
                ...
                <div class="mt-4">
                    <label for="payment_method_id" class="block text-sm font-medium text-stone-700 mb-1">
                        Moyen de paiement du remboursement <span class="text-red-500">*</span>
                    </label>
        ...
        {{-- Remboursement total --}}
        @php $remainingAmount = round((float) $order->total_amount - (float) $order->refunded_amount, 2); @endphp
        @if($remainingAmount > 0)
        <div class="bg-red-50 border border-red-200 rounded-xl p-5">
            <h2 class="font-semibold text-red-800 mb-2">Remboursement total</h2>
            <p class="text-sm text-red-700 mb-4">
                Rembourser la totalité du solde restant :
                <strong>{{ number_format($remainingAmount, 2, ',', ' ') }} €</strong>
                et débiter tous les points encore crédités.
            </p>
            <form action="{{ route('employee.refunds.store', $order) }}" method="POST"
                ...
                <div class="mb-4">
                    <label for="payment_method_id_total" class="block text-sm font-medium text-red-800 mb-1">
                        Moyen de paiement du remboursement <span class="text-red-500">*</span>
                    </label>
