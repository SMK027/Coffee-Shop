    ...
    <div class="bg-white rounded-xl shadow-sm border border-stone-100 overflow-hidden">
        ...
        @else
                        ...
                        <tr class="hover:bg-stone-50 transition-colors {{ $method->is_active ? '' : 'opacity-60' }}">
                            ...
                            <td class="px-5 py-3 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    {{-- Toggle activation (superviseur requis si non super admin) --}}
                                    <form action="{{ route('employee.payment-methods.toggle', $method) }}" method="POST"
                                          x-data="supervisorModal()" @submit.prevent="confirmThenSubmit($el)">
                                        @csrf
                                        @method('PATCH')
                                        @unless($isSuperAdmin)
            ...
            <div class="sm:hidden divide-y divide-stone-100">
                @foreach($methods as $method)
                <div class="px-4 py-4 {{ $method->is_active ? '' : 'opacity-60' }}">
                    ...
                    <p class="text-xs text-stone-500 font-mono">{{ $method->slug }}</p>
