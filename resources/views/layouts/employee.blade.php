    ...
    <div class="flex h-screen overflow-hidden">
            ...
            <nav class="flex-1 overflow-y-auto px-3 py-3 space-y-4">
                ...
                <div>
                    ...
                    <ul class="space-y-1 overflow-hidden transition-all duration-200" data-group-list="analyses">
                        ...
                        @endif
                        <li>
                            <a href="{{ route('employee.my-planning.index') }}"
                               class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors text-sm font-medium
                                      {{ request()->routeIs('employee.my-planning*') ? 'bg-amber-800 text-white' : 'text-amber-100 hover:bg-amber-800 hover:text-white' }}">
                                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div>
                    ...
                    <ul class="space-y-1 overflow-hidden transition-all duration-200" data-group-list="administration">
                        ...
                        <li>
                            <a href="{{ route('employee.plannings.index') }}"
                               class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors text-sm font-medium
                                      {{ request()->routeIs('employee.plannings*') ? 'bg-amber-800 text-white' : 'text-amber-100 hover:bg-amber-800 hover:text-white' }}">
                                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                                </svg>
                                Plannings
        ...
        <div class="flex-1 flex flex-col overflow-hidden min-w-0">
            ...
            @if(session()->has('impersonation.original_user_id'))
                <div class="px-4 sm:px-6 py-2.5 flex flex-wrap items-center justify-between gap-3 text-sm" style="background-color: #5b21b6; color: #ffffff;">
                    <span style="color: #ffffff;">Prise de contrôle active : vous utilisez le compte de {{ auth()->user()->name }}.</span>
                    <form action="{{ route('employee.users.release-control') }}" method="POST">
                        @csrf
                        <button type="submit" class="px-3 py-1.5 rounded font-medium" style="background-color: #ffffff; color: #4c1d95;">Reprendre mon compte</button>
                    </form>
                </div>
            @endif
