...
# ── Build APK release (JS bundlé, indépendant du serveur Metro) ──────
echo ""
echo "🔨  Build APK release en cours (assembleRelease)…"
echo "    (bundle JS inclus — pas besoin du serveur Expo)"
cd "$MOBILE_DIR/android"
./gradlew assembleRelease --no-daemon 2>&1 | tail -20

APK=$(find "$MOBILE_DIR/android/app/build/outputs/apk/release" -name "*.apk" 2>/dev/null | head -1)

if [ -n "$APK" ]; then
  echo ""
  echo "✅  APK généré :"
  echo "   $APK"
  cp "$APK" "$MOBILE_DIR/CoffeeShop-release.apk"
  echo "   → Copié dans : $MOBILE_DIR/CoffeeShop-release.apk"
else
